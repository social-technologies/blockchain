...
decl_storage! {
    trait Store for Module<T: Config> as ValidatorRegistry {
        GuardianOf get(fn social_of): map hasher(blake2_128_concat) T::AccountId => T::SocialTokenId;
...
decl_module! {
    pub struct Module<T: Config> for enum Call where origin: T::Origin {
        ...
        pub fn unregister(origin) -> dispatch::DispatchResult {
            ...
            ensure!(<GuardianOf<T>>::contains_key(&validator), Error::<T>::NotFound);

            let social_token_id = <GuardianOf<T>>::get(&validator);
