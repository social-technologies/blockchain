...
fn founding_works() {
	EnvBuilder::new().with_max_members(0).with_members(vec![]).execute(|| {
		...
		assert_eq!(SocialNetworkDao::founder(), None);
		assert_eq!(SocialNetworkDao::max_members(), 0);
		assert_eq!(SocialNetworkDao::pot(), 0);
		// Account 1 is set as the founder origin
		// Account 5 cannot start a society
		assert_noop!(SocialNetworkDao::found(Origin::signed(5), 20, 100, vec![]), BadOrigin);
		// Account 1 can start a society, where 10 is the founding member
		assert_ok!(SocialNetworkDao::found(Origin::signed(1), 10, 100, b"be cool".to_vec()));
		// SocialNetworkDao members only include 10
		assert_eq!(SocialNetworkDao::members(), vec![10]);
		// 10 is the head of the society
		assert_eq!(SocialNetworkDao::head(), Some(10));
		// ...and also the founder
		assert_eq!(SocialNetworkDao::founder(), Some(10));
		// 100 members max
		assert_eq!(SocialNetworkDao::max_members(), 100);
		// rules are correct
		assert_eq!(SocialNetworkDao::rules(), Some(blake2_256(b"be cool").into()));
		// Pot grows after first rotation period
		run_to_block(4);
		assert_eq!(SocialNetworkDao::pot(), 1000);
		// Cannot start another society
		assert_noop!(
			SocialNetworkDao::found(Origin::signed(1), 20, 100, vec![]),
...
fn unfounding_works() {
	EnvBuilder::new().with_max_members(0).with_members(vec![]).execute(|| {
		...
		assert_ok!(SocialNetworkDao::found(Origin::signed(1), 10, 100, vec![]));
		// Account 2 cannot unfound it as it's not the founder.
		assert_noop!(SocialNetworkDao::unfound(Origin::signed(2)), Error::<Test, _>::NotFounder);
		// Account 10 can, though.
		assert_ok!(SocialNetworkDao::unfound(Origin::signed(10)));

		// 1 sets the founder to 20 this time
		assert_ok!(SocialNetworkDao::found(Origin::signed(1), 20, 100, vec![]));
		// Bring in a new member...
		assert_ok!(SocialNetworkDao::bid(Origin::signed(10), 0));
		run_to_block(4);
		assert_ok!(SocialNetworkDao::vote(Origin::signed(20), 10, true));
		run_to_block(8);

		// Unfounding won't work now, even though it's from 20.
		assert_noop!(SocialNetworkDao::unfound(Origin::signed(20)), Error::<Test, _>::NotHead);
	});
}
...
fn basic_new_member_works() {
	EnvBuilder::new().execute(|| {
		...
		assert_ok!(SocialNetworkDao::bid(Origin::signed(20), 0));
		...
		assert_eq!(SocialNetworkDao::candidates(), vec![create_bid(0, 20, BidKind::Deposit(25))]);
		// 10 (a member) can vote for the candidate
		assert_ok!(SocialNetworkDao::vote(Origin::signed(10), 20, true));
		// Rotate period every 4 blocks
		run_to_block(8);
		// 20 is now a member of the society
		assert_eq!(SocialNetworkDao::members(), vec![10, 20]);
...
fn bidding_works() {
	EnvBuilder::new().execute(|| {
		...
		assert_ok!(SocialNetworkDao::bid(Origin::signed(60), 1900));
		assert_ok!(SocialNetworkDao::bid(Origin::signed(50), 500));
		assert_ok!(SocialNetworkDao::bid(Origin::signed(40), 400));
		assert_ok!(SocialNetworkDao::bid(Origin::signed(30), 300));
		// Rotate period
		run_to_block(4);
		// Pot is 1000 after "PeriodSpend"
		assert_eq!(SocialNetworkDao::pot(), 1000);
		assert_eq!(Balances::free_balance(SocialNetworkDao::account_id()), 10_000);
		// Choose smallest bidding users whose total is less than pot
		assert_eq!(SocialNetworkDao::candidates(), vec![
		...
		assert_ok!(SocialNetworkDao::vote(Origin::signed(10), 30, true));
		assert_ok!(SocialNetworkDao::vote(Origin::signed(10), 40, true));
		run_to_block(8);
		// Candidates become members after a period rotation
		assert_eq!(SocialNetworkDao::members(), vec![10, 30, 40]);
		// Pot is increased by 1000, but pays out 700 to the members
		assert_eq!(Balances::free_balance(SocialNetworkDao::account_id()), 9_300);
		assert_eq!(SocialNetworkDao::pot(), 1_300);
		// Left over from the original bids is 50 who satisfies the condition of bid less than pot.
		assert_eq!(SocialNetworkDao::candidates(), vec![ create_bid(500, 50, BidKind::Deposit(25)) ]);
		// 40, now a member, can vote for 50
		assert_ok!(SocialNetworkDao::vote(Origin::signed(40), 50, true));
		run_to_block(12);
		// 50 is now a member
		assert_eq!(SocialNetworkDao::members(), vec![10, 30, 40, 50]);
		// Pot is increased by 1000, and 500 is paid out. Total payout so far is 1200.
		assert_eq!(SocialNetworkDao::pot(), 1_800);
		assert_eq!(Balances::free_balance(SocialNetworkDao::account_id()), 8_800);
		// No more candidates satisfy the requirements
		assert_eq!(SocialNetworkDao::candidates(), vec![]);
		assert_ok!(SocialNetworkDao::defender_vote(Origin::signed(10), true)); // Keep defender around
		// Next period
		run_to_block(16);
		// Same members
		assert_eq!(SocialNetworkDao::members(), vec![10, 30, 40, 50]);
		// Pot is increased by 1000 again
		assert_eq!(SocialNetworkDao::pot(), 2_800);
		// No payouts
		assert_eq!(Balances::free_balance(SocialNetworkDao::account_id()), 8_800);
		// Candidate 60 now qualifies based on the increased pot size.
		assert_eq!(SocialNetworkDao::candidates(), vec![ create_bid(1900, 60, BidKind::Deposit(25)) ]);
		// Candidate 60 is voted in.
		assert_ok!(SocialNetworkDao::vote(Origin::signed(50), 60, true));
		run_to_block(20);
		// 60 joins as a member
		assert_eq!(SocialNetworkDao::members(), vec![10, 30, 40, 50, 60]);
		// Pay them
		assert_eq!(SocialNetworkDao::pot(), 1_900);
		assert_eq!(Balances::free_balance(SocialNetworkDao::account_id()), 6_900);
	});
}
...
fn unbidding_works() {
	EnvBuilder::new().execute(|| {
		// 20 and 30 make bids
		assert_ok!(SocialNetworkDao::bid(Origin::signed(20), 1000));
		assert_ok!(SocialNetworkDao::bid(Origin::signed(30), 0));
		...
		assert_noop!(SocialNetworkDao::unbid(Origin::signed(30), 1), Error::<Test, _>::BadPosition);
		// Can unbid themselves with the right position
		assert_ok!(SocialNetworkDao::unbid(Origin::signed(30), 0));
		// Balance is returned
		assert_eq!(Balances::free_balance(30), 50);
		assert_eq!(Balances::reserved_balance(30), 0);
		// 20 wins candidacy
		run_to_block(4);
		assert_eq!(SocialNetworkDao::candidates(), vec![ create_bid(1000, 20, BidKind::Deposit(25)) ]);
	});
}
...
fn payout_works() {
	EnvBuilder::new().execute(|| {
		...
		run_to_block(8);
		// payout not ready
		assert_noop!(SocialNetworkDao::payout(Origin::signed(20)), Error::<Test, _>::NoPayout);
		run_to_block(9);
		// payout should be here
		assert_ok!(SocialNetworkDao::payout(Origin::signed(20)));
...
fn basic_new_member_reject_works() {
	EnvBuilder::new().execute(|| {
		...
		assert_eq!(SocialNetworkDao::candidates(), vec![create_bid(0, 20, BidKind::Deposit(25))]);
		// We say no
		assert_ok!(SocialNetworkDao::vote(Origin::signed(10), 20, false));
		run_to_block(8);
		// User is not added as member
		assert_eq!(SocialNetworkDao::members(), vec![10]);
		// User is suspended
		assert_eq!(SocialNetworkDao::candidates(), vec![]);
		assert_eq!(SocialNetworkDao::suspended_candidate(20).is_some(), true);
	});
}
...
fn slash_payout_works() {
	EnvBuilder::new().execute(|| {
		...
		assert_noop!(SocialNetworkDao::payout(Origin::signed(20)), Error::<Test, _>::NoPayout);
		// slash payout
		assert_eq!(SocialNetworkDao::slash_payout(&20, 500), 500);
...
fn slash_payout_multi_works() {
	EnvBuilder::new().execute(|| {
		...
		SocialNetworkDao::bump_payout(&20, 5, 100);
		SocialNetworkDao::bump_payout(&20, 10, 100);
		SocialNetworkDao::bump_payout(&20, 15, 100);
		SocialNetworkDao::bump_payout(&20, 20, 100);
		// payouts in queue
		assert_eq!(Payouts::<Test>::get(20), vec![(5, 100), (10, 100), (15, 100), (20, 100)]);
		// slash payout
		assert_eq!(SocialNetworkDao::slash_payout(&20, 250), 250);
		assert_eq!(Payouts::<Test>::get(20), vec![(15, 50), (20, 100)]);
		// slash again
		assert_eq!(SocialNetworkDao::slash_payout(&20, 50), 50);
...
fn suspended_member_life_cycle_works() {
	EnvBuilder::new().execute(|| {
		...
		assert_ok!(SocialNetworkDao::add_member(&20));
		...
		assert_ok!(SocialNetworkDao::bid(Origin::signed(40), 0));
		run_to_block(16);
		...
		SocialNetworkDao::bump_payout(&20, 10, 100);
		assert_noop!(SocialNetworkDao::payout(Origin::signed(20)), Error::<Test, _>::NotMember);

		// Normal people cannot make judgement
		assert_noop!(SocialNetworkDao::judge_suspended_member(Origin::signed(20), 20, true), BadOrigin);

		// Suspension judgment origin can judge thee
		// Suspension judgement origin forgives the suspended member
		assert_ok!(SocialNetworkDao::judge_suspended_member(Origin::signed(2), 20, true));
		assert_eq!(<SuspendedMembers<Test>>::get(20), false);
		assert_eq!(<Members<Test>>::get(), vec![10, 20]);

		// Let's suspend them again, directly
		SocialNetworkDao::suspend_member(&20);
		assert_eq!(<SuspendedMembers<Test>>::get(20), true);
		// Suspension judgement origin does not forgive the suspended member
		assert_ok!(SocialNetworkDao::judge_suspended_member(Origin::signed(2), 20, false));
		// Cleaned up
		assert_eq!(<SuspendedMembers<Test>>::get(20), false);
		assert_eq!(<Members<Test>>::get(), vec![10]);
...
fn suspended_candidate_rejected_works() {
	EnvBuilder::new().execute(|| {
		// Starting Balance
		assert_eq!(Balances::free_balance(20), 50);
		assert_eq!(Balances::free_balance(SocialNetworkDao::account_id()), 10000);
		// 20 makes a bid
		assert_ok!(SocialNetworkDao::bid(Origin::signed(20), 0));
		assert_eq!(Balances::free_balance(20), 25);
		assert_eq!(Balances::reserved_balance(20), 25);
		...
		assert_noop!(SocialNetworkDao::judge_suspended_candidate(Origin::signed(20), 20, Judgement::Approve), BadOrigin);

		// Suspension judgement origin makes no direct judgement
		assert_ok!(SocialNetworkDao::judge_suspended_candidate(Origin::signed(2), 20, Judgement::Rebid));
		...
		assert_ok!(SocialNetworkDao::judge_suspended_candidate(Origin::signed(2), 20, Judgement::Reject));
		// User is slashed
		assert_eq!(Balances::free_balance(20), 25);
		assert_eq!(Balances::reserved_balance(20), 0);
		// Funds are deposited to society account
		assert_eq!(Balances::free_balance(SocialNetworkDao::account_id()), 10025);
		// Cleaned up
		assert_eq!(SocialNetworkDao::candidates(), vec![]);
...
fn vouch_works() {
	EnvBuilder::new().execute(|| {
		...
		assert_noop!(SocialNetworkDao::vouch(Origin::signed(1), 20, 1000, 100), Error::<Test, _>::NotMember);
		// A member can though
		assert_ok!(SocialNetworkDao::vouch(Origin::signed(10), 20, 1000, 100));
		assert_eq!(<Vouching<Test>>::get(10), Some(VouchingStatus::Vouching));
		// A member cannot vouch twice at the same time
		assert_noop!(SocialNetworkDao::vouch(Origin::signed(10), 30, 100, 0), Error::<Test, _>::AlreadyVouching);
		...
		run_to_block(4);
		assert_eq!(SocialNetworkDao::candidates(), vec![create_bid(1000, 20, BidKind::Vouch(10, 100))]);
		// Vote yes
		assert_ok!(SocialNetworkDao::vote(Origin::signed(10), 20, true));
...
fn voucher_cannot_win_more_than_bid() {
	EnvBuilder::new().execute(|| {
		...
		assert_ok!(SocialNetworkDao::vouch(Origin::signed(10), 20, 100, 1000));
		// Vouching creates the right kind of bid
		assert_eq!(<Bids<Test>>::get(), vec![create_bid(100, 20, BidKind::Vouch(10, 1000))]);
		// Vouched user can become candidate
		run_to_block(4);
		assert_eq!(SocialNetworkDao::candidates(), vec![create_bid(100, 20, BidKind::Vouch(10, 1000))]);
		// Vote yes
		assert_ok!(SocialNetworkDao::vote(Origin::signed(10), 20, true));
		// Vouched user can win
		run_to_block(8);
...
fn unvouch_works() {
	EnvBuilder::new().execute(|| {
		...
		assert_eq!(SocialNetworkDao::members(), vec![10]);
		// 10 vouches for 20
		assert_ok!(SocialNetworkDao::vouch(Origin::signed(10), 20, 100, 0));
		// 20 has a bid
		assert_eq!(<Bids<Test>>::get(), vec![create_bid(100, 20, BidKind::Vouch(10, 0))]);
		// 10 is vouched
		assert_eq!(<Vouching<Test>>::get(10), Some(VouchingStatus::Vouching));
		// To unvouch, you must know the right bid position
		assert_noop!(SocialNetworkDao::unvouch(Origin::signed(10), 2), Error::<Test, _>::BadPosition);
		// 10 can unvouch with the right position
		assert_ok!(SocialNetworkDao::unvouch(Origin::signed(10), 0));
		...
		assert_ok!(SocialNetworkDao::vouch(Origin::signed(10), 20, 100, 0));
		run_to_block(4);
		assert_eq!(SocialNetworkDao::candidates(), vec![create_bid(100, 20, BidKind::Vouch(10, 0))]);
		assert_noop!(SocialNetworkDao::unvouch(Origin::signed(10), 0), Error::<Test, _>::BadPosition);
		// 10 is still vouching until candidate is approved or rejected
		assert_eq!(<Vouching<Test>>::get(10), Some(VouchingStatus::Vouching));
		run_to_block(8);
		// In this case candidate is denied and suspended
		assert!(SocialNetworkDao::suspended_candidate(&20).is_some());
		assert_eq!(SocialNetworkDao::members(), vec![10]);
		...
		assert_noop!(SocialNetworkDao::unvouch(Origin::signed(10), 0), Error::<Test, _>::NotVouching);
	});
}
...
fn unbid_vouch_works() {
	EnvBuilder::new().execute(|| {
		...
		assert_ok!(SocialNetworkDao::unbid(Origin::signed(20), 0));
...
fn founder_and_head_cannot_be_removed() {
	EnvBuilder::new().execute(|| {
		...
		// Replace the head
		assert_ok!(SocialNetworkDao::bid(Origin::signed(50), 0));
		run_to_block(28);
		assert_ok!(SocialNetworkDao::vote(Origin::signed(10), 50, true));
		assert_ok!(SocialNetworkDao::defender_vote(Origin::signed(10), true)); // Keep defender around
		run_to_block(32);
		assert_eq!(SocialNetworkDao::members(), vec![10, 50]);
		assert_eq!(SocialNetworkDao::head(), Some(50));
		// Founder is unchanged
		assert_eq!(SocialNetworkDao::founder(), Some(10));

		// 50 can still accumulate strikes
		assert_ok!(SocialNetworkDao::bid(Origin::signed(60), 0));
		run_to_block(40);
		assert_eq!(Strikes::<Test>::get(50), 1);
		assert_ok!(SocialNetworkDao::bid(Origin::signed(70), 0));
		run_to_block(48);
		assert_eq!(Strikes::<Test>::get(50), 2);

		// Replace the head
		assert_ok!(SocialNetworkDao::bid(Origin::signed(80), 0));
		run_to_block(52);
		assert_ok!(SocialNetworkDao::vote(Origin::signed(10), 80, true));
		assert_ok!(SocialNetworkDao::vote(Origin::signed(50), 80, true));
		assert_ok!(SocialNetworkDao::defender_vote(Origin::signed(10), true)); // Keep defender around
		run_to_block(56);
		assert_eq!(SocialNetworkDao::members(), vec![10, 50, 80]);
		assert_eq!(SocialNetworkDao::head(), Some(80));
		assert_eq!(SocialNetworkDao::founder(), Some(10));

		// 50 can now be suspended for strikes
		assert_ok!(SocialNetworkDao::bid(Origin::signed(90), 0));
		run_to_block(60);
		// The candidate is rejected, so voting approve will give a strike
		assert_ok!(SocialNetworkDao::vote(Origin::signed(50), 90, true));
		run_to_block(64);
		assert_eq!(Strikes::<Test>::get(50), 0);
		assert_eq!(<SuspendedMembers<Test>>::get(50), true);
		assert_eq!(SocialNetworkDao::members(), vec![10, 80]);
	});
}
...
fn challenges_work() {
	EnvBuilder::new().execute(|| {
		// Add some members
		assert_ok!(SocialNetworkDao::add_member(&20));
		assert_ok!(SocialNetworkDao::add_member(&30));
		assert_ok!(SocialNetworkDao::add_member(&40));
		// Votes are empty
		...
		assert_eq!(SocialNetworkDao::members(), vec![10, 20, 30, 40]);
		assert_eq!(SocialNetworkDao::defender(), None);
		// 20 will be challenged during the challenge rotation
		run_to_block(8);
		assert_eq!(SocialNetworkDao::defender(), Some(30));
		// They can always free vote for themselves
		assert_ok!(SocialNetworkDao::defender_vote(Origin::signed(30), true));
		...
		assert_noop!(SocialNetworkDao::defender_vote(Origin::signed(1), true), Error::<Test, _>::NotMember);
		// 3 people say accept, 1 reject
		assert_ok!(SocialNetworkDao::defender_vote(Origin::signed(10), true));
		assert_ok!(SocialNetworkDao::defender_vote(Origin::signed(20), true));
		assert_ok!(SocialNetworkDao::defender_vote(Origin::signed(30), true));
		assert_ok!(SocialNetworkDao::defender_vote(Origin::signed(40), false));
		run_to_block(24);
		// 20 survives
		assert_eq!(SocialNetworkDao::members(), vec![10, 20, 30, 40]);
		// Votes are reset
		...
		assert_ok!(SocialNetworkDao::defender_vote(Origin::signed(10), true));
		assert_ok!(SocialNetworkDao::defender_vote(Origin::signed(20), true));
		assert_ok!(SocialNetworkDao::defender_vote(Origin::signed(30), false));
		assert_ok!(SocialNetworkDao::defender_vote(Origin::signed(40), false));
		run_to_block(32);
		// 20 is suspended
		assert_eq!(SocialNetworkDao::members(), vec![10, 20, 40]);
		assert_eq!(SocialNetworkDao::suspended_member(30), true);
		// New defender is chosen
		assert_eq!(SocialNetworkDao::defender(), Some(20));
		// Votes are reset
		assert_eq!(<DefenderVotes<Test>>::get(10), None);
		assert_eq!(<DefenderVotes<Test>>::get(20), None);
		assert_eq!(<DefenderVotes<Test>>::get(30), None);
...
fn bad_vote_slash_works() {
	EnvBuilder::new().execute(|| {
		...
		assert_ok!(SocialNetworkDao::add_member(&40));
		// Create some payouts
		SocialNetworkDao::bump_payout(&10, 5, 100);
		SocialNetworkDao::bump_payout(&20, 5, 100);
		SocialNetworkDao::bump_payout(&30, 5, 100);
		SocialNetworkDao::bump_payout(&40, 5, 100);
		// Check starting point
		assert_eq!(SocialNetworkDao::members(), vec![10, 20, 30, 40]);
		...
		// Create a new bid
		assert_ok!(SocialNetworkDao::bid(Origin::signed(50), 1000));
		run_to_block(4);
		assert_ok!(SocialNetworkDao::vote(Origin::signed(10), 50, false));
		assert_ok!(SocialNetworkDao::vote(Origin::signed(20), 50, true));
		assert_ok!(SocialNetworkDao::vote(Origin::signed(30), 50, false));
		assert_ok!(SocialNetworkDao::vote(Origin::signed(40), 50, false));
		run_to_block(8);
...
fn user_cannot_bid_twice() {
	EnvBuilder::new().execute(|| {
		// Cannot bid twice
		assert_ok!(SocialNetworkDao::bid(Origin::signed(20), 100));
		assert_noop!(SocialNetworkDao::bid(Origin::signed(20), 100), Error::<Test, _>::AlreadyBid);
		// Cannot bid when vouched
		assert_ok!(SocialNetworkDao::vouch(Origin::signed(10), 30, 100, 100));
		assert_noop!(SocialNetworkDao::bid(Origin::signed(30), 100), Error::<Test, _>::AlreadyBid);
		// Cannot vouch when already bid
		assert_ok!(SocialNetworkDao::add_member(&50));
		assert_noop!(SocialNetworkDao::vouch(Origin::signed(50), 20, 100, 100), Error::<Test, _>::AlreadyBid);
	});
}
...
fn vouching_handles_removed_member_with_bid() {
	EnvBuilder::new().execute(|| {
		...
		assert_ok!(SocialNetworkDao::vouch(Origin::signed(20), 30, 1000, 100));
...
fn vouching_handles_removed_member_with_candidate() {
	EnvBuilder::new().execute(|| {
		...
		run_to_block(4);
		assert_eq!(SocialNetworkDao::candidates(), vec![create_bid(1000, 30, BidKind::Vouch(20, 100))]);
		// Suspend that member
		SocialNetworkDao::suspend_member(&20);
		assert_eq!(<SuspendedMembers<Test>>::get(20), true);
		// Nothing changes yet
		...
		assert_ok!(SocialNetworkDao::vote(Origin::signed(10), 30, true));
		run_to_block(8);
		assert_eq!(SocialNetworkDao::members(), vec![10, 30]);
...
fn votes_are_working() {
	EnvBuilder::new().execute(|| {
		...
		assert_noop!(SocialNetworkDao::vote(Origin::signed(10), 50, true), Error::<Test, _>::NotCandidate);
		// Votes are stored
...
fn max_limits_work() {
	EnvBuilder::new().with_pot(100000).execute(|| {
		...
		for i in (100..1110).rev() {
			...
			assert_ok!(SocialNetworkDao::bid(Origin::signed(i as u128), i));
		}
		...
		assert_eq!(SocialNetworkDao::candidates().len(), 10);
		// Fill up membership, max 100, we will do just 95
		for i in 2000..2095 {
			assert_ok!(SocialNetworkDao::add_member(&(i as u128)));
		}
		// Remember there was 1 original member, so 96 total
		assert_eq!(SocialNetworkDao::members().len(), 96);
		// Rotate period
		run_to_block(8);
		// Only of 4 candidates possible now
		assert_eq!(SocialNetworkDao::candidates().len(), 4);
		// Fill up members with suspended candidates from the first rotation
		for i in 100..104 {
			assert_ok!(SocialNetworkDao::judge_suspended_candidate(Origin::signed(2), i, Judgement::Approve));
		}
		assert_eq!(SocialNetworkDao::members().len(), 100);
		// Can't add any more members
		assert_noop!(SocialNetworkDao::add_member(&98), Error::<Test, _>::MaxMembers);
		...
		for i in SocialNetworkDao::members().iter() {
			assert_ok!(SocialNetworkDao::vote(Origin::signed(*i), 110, true));
			assert_ok!(SocialNetworkDao::vote(Origin::signed(*i), 111, true));
			assert_ok!(SocialNetworkDao::vote(Origin::signed(*i), 112, true));
		}
		// Rotate period
		run_to_block(12);
		// Members length is over 100, no problem...
		assert_eq!(SocialNetworkDao::members().len(), 103);
		// No candidates because full
		assert_eq!(SocialNetworkDao::candidates().len(), 0);
		// Increase member limit
		assert_ok!(SocialNetworkDao::set_max_members(Origin::root(), 200));
		// Rotate period
		run_to_block(16);
		// Candidates are back!
		assert_eq!(SocialNetworkDao::candidates().len(), 10);
...
fn zero_bid_works() {
	...
	EnvBuilder::new().execute(|| {
		// Users make bids of various amounts
		assert_ok!(SocialNetworkDao::bid(Origin::signed(60), 400));
		assert_ok!(SocialNetworkDao::bid(Origin::signed(50), 300));
		assert_ok!(SocialNetworkDao::bid(Origin::signed(30), 0));
		assert_ok!(SocialNetworkDao::bid(Origin::signed(20), 0));
		assert_ok!(SocialNetworkDao::bid(Origin::signed(40), 0));
		...
		assert_ok!(SocialNetworkDao::vote(Origin::signed(10), 30, true));
		assert_ok!(SocialNetworkDao::vote(Origin::signed(10), 50, true));
		assert_ok!(SocialNetworkDao::vote(Origin::signed(10), 60, true));
		run_to_block(8);
		// Candidates become members after a period rotation
		assert_eq!(SocialNetworkDao::members(), vec![10, 30, 50, 60]);
		// The zero bid is selected as head
		assert_eq!(SocialNetworkDao::head(), Some(30));
	});
}
	...
	EnvBuilder::new().execute(|| {
		for i in 0..5 {
			for j in 0..5 {
				...
				assert_ok!(SocialNetworkDao::bid(Origin::signed(100 + (i * 5 + j) as u128), j));
			}
		}
