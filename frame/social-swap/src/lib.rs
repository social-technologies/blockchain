...
use sp_runtime::{
	RuntimeDebug, DispatchResult, DispatchError, ModuleId, Perbill,
	traits::{
		CheckedAdd, CheckedSub, CheckedMul, CheckedDiv, MaybeSerializeDeserialize,
		Member, One, Saturating, AtLeast32Bit, Zero, Bounded, SaturatedConversion,
		AtLeast32BitUnsigned, AccountIdConversion, Convert, UniqueSaturatedInto
	}
};
use sp_std::{prelude::*, ops::Div};
use sp_std::{cmp, convert::TryFrom, result, fmt::Debug};
use frame_support::{
...
use pallet_social_tokens::{TokenDossier, Fungible, IssueAndBurn};
...
impl<T: Trait> Module<T> {
	...
	fn native_to_trade_input(
		...
		ensure!(native_in > Zero::zero() && min_trade_tokens > Zero::zero(), Error::<T>::TooLow);
	...
	fn trade_to_native_input(
		...
		ensure!(trade_in > Zero::zero() && min_native_tokens > Zero::zero(), Error::<T>::TooLow);
